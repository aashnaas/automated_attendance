...
window = tk.Tk()    #To initialize tkinter, we have to create a Tk root widget,
#which is a window with a title bar and other decoration provided by the window
#manager. The root widget has to be
#created before any other widgets and there can only be one root widget.
window.title("Automatic Attendance Management System")
...
def manually_fill():
    ...
    def fill_attendance():
        ...
        try:
            global cursor
            connection = pymysql.connect(host='localhost', user='root', password='anchal@123', db='test')
        ...
        if subb=='':
            ...
            def create_csv():
                ...
                csv_name='C:\\Users\\hp\\Desktop\\MiniPro\\Attendace_management_system-master'+DB_table_name+'.csv'
    ...
    def Fillattendances():
        ...
        if time.time() < future:
            if sub == '':
                ...
                root.configure(background='snow')
                cs ='C:\\Users\\hp\\Desktop\\MiniPro\\Attendace_management_system-master\\Attendace_management_system-master' + fileName
...
message = tk.Label(window, text="Automatic Attendance Management System", bg="cyan", fg="black", width=50,
